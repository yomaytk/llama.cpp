...
void llama_model_qwen3next::load_arch_tensors(llama_model_loader & ml) {
    ...
    auto load_block_mtp = [&](int il) {
        // MTP layers are flagged non-recurrent, so the attention + FFN tensor set is
        // identical to a trunk full-attention block. The MTP block's own tensors are
        // always required, even in an mtp-only GGUF.
...
llama_model_qwen3next::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    // Prune rows at the last layer unless the MTP head needs the unmasked
    // pre-head hidden state for all tokens (embeddings_nextn && !masked).
    const bool prune_last_layer = !cparams.embeddings_nextn || cparams.embeddings_nextn_masked;
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (il == n_layer - 1 && inp_out_ids && prune_last_layer) {
    ...
    if (!prune_last_layer && inp_out_ids) {
        cur = ggml_get_rows(ctx0, cur, inp_out_ids);
    }
