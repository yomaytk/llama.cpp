...
fn main(
    ...
    let cache_off = params.dst_fuse_off;
#else
    let state_size_per_snap = state_size * params.h * params.n_seqs;
    let cache_off = params.s_off;
#endif
    ...
    for (var t = 0u; t < params.n_tokens; t++) {
        ...
        if (params.K > 1u) {
            ...
            if (target_slot >= 0 && target_slot < i32(params.K)) {
                let slot_base = cache_off + u32(target_slot) * state_size_per_snap + state_out_base;
                for (var i = 0u; i < S_V; i++) {
    ...
    if (params.K == 1u) {
        for (var i = 0u; i < S_V; i++) {
            DST_SNAP[cache_off + state_out_base + col * S_V + i] = state[i];
        }
    }
}
